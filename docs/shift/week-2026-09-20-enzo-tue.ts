...
// ירין ביקש שוטף שלישי לפסאו בשלישי 22/09; טוני לא זמין, אז כפולה של אנזו.
// אנזו כבר עובד ג' בוקר באומינו 10:00 -> מוסיפים לו פסאו ערב 19:00.
// פסאו ערב שלישי יעבור מ-2 ל-3: הילארי 17:00 · ג'ונתן 18:00 · אנזו 19:00.
// רביעי לא נכלל כאן — אנזו כבר בערב של רביעי, ראה הודעה לירין.
// הרוטה נשארת טיוטה.
...
Deno.serve(async (req) => {
  try {
    ...
    const dup = live.find((c: any) => c.day === TUE && c.shift === EREV
      && Number(c.employee) === ENZO && hhmm(c.planned_start) === START);
    if (dup) return new Response(JSON.stringify({ skipped: "אנזו כבר משובץ ג' ערב 19:00", id: dup.id }, null, 2),
      { headers: { "Content-Type": "application/json" } });

    const date = dateOf(TUE);
    const orders = new Set(live.filter((c: any) => c.day === TUE).map((c: any) => Number(c.order || 0)));
    let order = 1; while (orders.has(order)) order++;

    if (!go) {
      return new Response(JSON.stringify({ dry: true, would_create: { date, emp: ENZO, start: START, order },
        tuesday_now: live.filter((c: any) => c.day === TUE).map((c: any) =>
          `${hhmm(c.planned_start)} ${c.employee}${isU(c) ? " (אומינו)" : ""}${c.shift === BOKER ? " [בוקר]" : " [ערב]"}`) }, null, 2),
        { headers: { "Content-Type": "application/json" } });
    }

    const payload = {
      rota: P_ROTA, sub_rota: null, shift: EREV, day: TUE, date, role: DISH, employee: ENZO, order,
    ...
    return new Response(JSON.stringify({ created: 1, is_published: pRota?.is_published ?? null,
      dish_cells: after.length, per_person: per, tuesday: by[dateOf(TUE)] }, null, 2),
      { headers: { "Content-Type": "application/json" } });
  } catch (e) {
